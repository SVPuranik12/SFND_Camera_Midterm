...
int main(int argc, const char *argv[])
{
    ...
    string DetType, DescType;
    double DetTime = 0,DescTime = 0;
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        {
            /* Remove the very first element which is in turn old element in the buffer 
            and start insertion of new element*/
        ...
        float Cx = 535, Cy = 180, W = 180, H = 150; // Taking co-ordinates for bounding box of preceeding vehicle
        if (bFocusOnVehicle)
        {
         ...
         {
            //check if the key point is in the box
            if((keypoints[i].pt.x > Cx && keypoints[i].pt.x < Cx+W) && (keypoints[i].pt.y > Cy && keypoints[i].pt.y < Cy+H))
             { 
        ...
        bool bLimitKpts = true; // limit the keypoints
        if (bLimitKpts)
        {
            int maxKeypoints = 50;
